...
# Replace the torchvision imports with manual implementations
def build_transform(input_size, norm_type="imagenet"):
    ...
    def transform(img):
        ...
        # Convert to tensor (equivalent to ToTensor)
...
class EagleProcessor:
    def __init__(
        ...
        if model_path is None:
            # if no model path installed we will use the model in the current directory
            model_path = os.path.dirname(os.path.abspath(__file__))

        if model_path.endswith("/"):
            model_path = model_path[:-1]

        self.model_spec = ModelSpecificValues(**model_spec)
